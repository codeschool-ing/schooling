#!/usr/bin/env bash
# The terminal sessions quoted in lesson 6 of virtualization, as a script that produces them.
#
...
block pools
on host 'virsh pool-list --all --details'
on host 'virsh vol-list images --details'

block bridge
on host 'ip -br link show type bridge'
on host 'bridge link'

block autostart
on host 'virsh autostart vm1'
on host 'virsh dominfo vm1 | grep -E "^(Name|State|Autostart)"'
