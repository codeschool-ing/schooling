...
A forma com palavra-chave — `name=("column", "function")` — é a de aprender. Ela nomeia as colunas
de saída, coisa que a forma antiga com lista não faz, e se lê como a tabela que produz.

## O `count` conta o que está presente

O `orders` acima é o `count` de `id`, que não tem lacunas. Se fosse o `count` de `cents`, o BR
