...
`class Student(Person)` diz que um `Student` é uma `Person`. Ele ganha todo método que `Person` tem, e
sobrescrever um é simplesmente defini-lo de novo.

## O `super()`

`super().greet()` chama a versão acima desta na cadeia. Sem ele, uma sobrescrita SUBSTITUI o
...
`Student` é uma `Person` — todo método que recebe uma `Person` funciona com um.
