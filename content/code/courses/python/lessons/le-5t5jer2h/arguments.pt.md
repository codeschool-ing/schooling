...
conectar("db.example.tld")
conectar("db.example.tld", 6543)
conectar("db.example.tld", timeout=30)
```
...
`conectar("db.example.tld", 6543)` se lê bem porque um host e uma porta são obviamente um host e
uma porta. `cobrar(conta, 1200, True)` não se lê, nem por quem escreveu.
...
```python
def acrescentar(item, cesta=[]):    # UMA lista, pela vida inteira do programa
    cesta.append(item)
    return cesta

acrescentar("maçã")     # ['maçã']
acrescentar("pera")     # ['maçã', 'pera']   ← a mesma lista
```
...
```python
def acrescenta_um(xs):
    xs.append(1)      # a lista de quem chamou muda

def religa(xs):
    xs = [1]          # só o nome local muda
```
