#!/usr/bin/env bash
# The terminal sessions quoted in lesson 2 of tech-support, as a script that produces them.
#
...
# user bruno on pc1; two print queues, office and pdf, both writing to
# /dev/null (FileDevice Yes in cups-files.conf), with office as the system's
# default; and WHAT BRUNO DID, as bruno: lpoptions -d pdf, then two prints of
# report.txt. A queue that writes to /dev/null stands in for a real printer
# and a real PDF writer: the lab has neither, and the lesson is about where
# the jobs went, not about paper.
...
quiet pc1 'useradd -m -s /bin/bash bruno; echo "FileDevice Yes" >> /etc/cups/cups-files.conf; systemctl restart cups; for q in office pdf; do lpadmin -p $q -E -v file:///dev/null; done; lpadmin -d office'
quiet pc1 'sudo -u bruno bash -c "printf \"sales, September\n\" > ~/report.txt; lpoptions -d pdf >/dev/null; lp ~/report.txt >/dev/null; lp ~/report.txt >/dev/null"'

block works-for-me
on pc1 'lpstat -d; echo "a test page" | lp'

block as-the-user
on pc1 'sudo -u bruno lpstat -d'
on pc1 'sudo -u bruno bash -c "cd ~ && lp report.txt"'

block evidence
on pc1 'sudo lpstat -W completed -o'
on pc1 'sudo cat /home/bruno/.cups/lpoptions; sudo stat -c "%y" /home/bruno/.cups/lpoptions'

block fix
on pc1 'sudo -u bruno lpoptions -d office >/dev/null && sudo -u bruno lpstat -d'
on pc1 'sudo -u bruno bash -c "cd ~ && lp report.txt"'
