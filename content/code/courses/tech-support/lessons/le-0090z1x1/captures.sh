#!/usr/bin/env bash
# The terminal sessions quoted in lesson 4 of tech-support, as a script that produces them.
#
...
# 2", standing for what a real printer reports when it stops itself. Nothing listens
# on port 8080 and no name intranet is known on pc1, on purpose.
...
quiet pc1 'useradd -m -s /bin/bash elisa; echo "FileDevice Yes" >> /etc/cups/cups-files.conf; systemctl restart cups; lpadmin -p office -E -v file:///dev/null; lpadmin -d office; cupsdisable -r "paper jam, tray 2" office'

block messages
on pc1 'curl -sS -m 10 http://intranet/'
on pc1 'sudo -u elisa cat /etc/shadow'
on pc1 'curl -sS -m 10 http://localhost:8080/'
