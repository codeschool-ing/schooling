...
Lesson 3 section 49 said `/proc` has a directory per process and that everything in it is a file.
...
**PID 1 is special and the rest are not.** Lesson 5 section 77 covered process one. Nothing else
...
server running as `www-data` is lesson 5 section 76's sentence, and this column is where you see
it.

It also decides what **you** may do to it. You can signal your own processes. Signalling somebody
else's needs root, and section 94 shows the refusal.
...
section 92's `top` counts threads, which is why a machine with eighty processes can report several
...
**A zombie.** A process that has finished and is still in the table. Section 89 makes one.
