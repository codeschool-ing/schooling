...
A web server. A database. The ssh server you used in section 75 — **something had to be listening
...
it does not stop it — and why its output has to go somewhere else, which is section 81's whole
subject.
...
**It runs as its own account.** Section 71 counted thirty accounts and one person, and this is what
...
detaching itself** — which is why `Type=simple` in section 80 is the common case, and why a program
...
| | |
|---|---|
| `/etc/<name>/` | its configuration — text, section 37 |
...
| `/usr/lib/systemd/system/<name>.service` | how it is started — section 80 |

So: **`nginx` is five paths you can guess before you look.** That is the payoff of lesson 3's
section 37 being a standard rather than a habit.

## Three things a service needs that a command does not

**Something to start it at boot.** That is the whole of sections 77 and 78, and the reason the
...
line it writes has to be collected by something — a log file, or the journal in section 81.
...
watching. `Restart=on-failure` in section 80 is that, in one line.
...
The third disappears when your session ends, unless you take steps — lesson 6 section 96 is about
