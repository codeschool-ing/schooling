...
| [`docs/EXECUTOR.md`](docs/EXECUTOR.md) | what it would take to run a student's program: why `code` and `expected-output` have no grader, the two systems the word "sandbox" is doing duty for, the isolation and the quota, and why the order is to write the material that needs no runtime first |
...
tools/        the checks and the jobs — a11y-test, graph-test, landing-test,
              console-test, mfa-test, fetch-once, check-interface, check-css,
              check-origin, check-highlight, validate-content, check-exercises,
              figure-fit, term-capture, bundle, bundle-test, restore-drill,
              release, fonts, world
