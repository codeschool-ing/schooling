...
- [ ] Pix Automático, deliberately after the first two
  ...
  *Pix carries a five per cent discount, and it pays for about a third of itself.* The numbers are now the sandbox's rather than memory's: **R$ 0,99 flat** on a Pix or boleto received, against **1,99% + R$ 0,49** on a card. On the same R$ 590 sale that is R$ 0,99 against R$ 12,23 — an eleven-real difference, so a discount costing R$ 29,50 really costs about R$ 18. Less of a free lunch than the 2,99% guessed at above, and still worth it, because the fee is not the whole of what is being bought: **Pix settles in seconds and the card settles in 32 days**, per instalment. A six-way split paid on a card is the last sixth of that sale arriving seven months after the student started studying. That is the argument for the discount, and it got stronger rather than weaker when the real table appeared.

  Treat those two rates as indicative — they are what the sandbox shows a new account, not a table anybody negotiated. What the sandbox did NOT show is the interest table for instalments beyond six, which is the number the paragraph above is waiting for; it is not in the charge screen, so it is still a question for the account rather than an answer from it.
