...
- [x] Email deliverability — bounces and complaints — **the mechanism, and not the screen; the box is ticked on what it says and `docs/CONSOLE.md` still lists the screen among what is not built.** The platform started sending in v0.9.0 and was deaf until v0.10.0: an address that refused us permanently went on being written to, and every attempt after the first is a mark against this domain with the providers who decide whether ANYBODY else's mail arrives. One unreachable address costs one person a link; a hundred attempts at it costs every future student their inbox.
  ...
  **Only what is permanent**, checked at the hook and again at the list. A soft bounce is not a refusal, and treating one as permanent would have suppressed every address at a whole provider during the Proton outage of 27 August 2026 — which is not a hypothetical, it is the day before this shipped. Unsubscribing is absent for its own reason: refusing somebody the link that proves they own their own address, because they once opted out of a newsletter that does not exist, would be obeying an instruction nobody gave.
  ...
  THE SECRET IS IN THE PATH BECAUSE THE PROVIDER DOES NOT SIGN THE BODY, compared in constant time, with a 404 rather than a 403 for a wrong one — a 403 tells a scanner they have found the endpoint and need only the secret. And `web.Logger` writes `r.URL.Path` on every request, so `web.Loggable` redacts everything under `/hooks/`: without it the one measure protecting the endpoint would have gone to Cloud Logging in plain text on the first delivery and stayed there. Proven in production rather than argued — both requests logged as `/hooks/mail/...`, and the refusal recorded with a reason and no address.

  **Two things this found and did not fix, both in the follow-up rather than smuggled in here.** The provider's newer webhook form offers header authentication, so the premise written into the code — that there is nothing to verify against — is out of date; a header would not appear in a log, a URL, or a screenshot. And there is an `invalid` event, permanent, that `permanent()` does not know: leaving it out was an omission rather than a decision, and the configuration and the code have to agree about what is covered
