...
- [ ] The funnel, all eight steps from *arrived* to *subscribed* — **six of the eight are now emitted, and the two that are not cannot be.** `visitor.arrived` is the one that mattered most: it is the only step that cannot be reconstructed afterwards, because by the time somebody signs up the visit that brought them is over. Also added: `track.opened` (there is no enrolment, so choosing a track is opening one), `lesson.opened` and `course.completed` — the last derived where the completion happens, because computed later it would be a query over the catalogue as it is TODAY and would answer wrongly the first time a section was added to a course.

  **Verifying the address has no event because it has no feature**: `email_verified_at` exists and nothing writes it. **Subscribing has no event because nothing creates a subscription** — that waits on the gateway. Neither is faked, and the funnel will have to say so rather than show a step at zero, which reads as everybody dropping out
