...
- [ ] The closed list of system parameters, each change audited with actor, old and new value — **two parameters exist now and the list still does not.** A school's accent is set from `Operate → Schools`, recorded with the actor, the colour that was there and the colour that replaced it, and refused for a role below operator. The audit seam grew to make that possible: it carried one value and a bare account id, so every entry the console wrote said "account" and put whatever it had in `before`; it now names the SUBJECT'S KIND and both sides of the change.
  ...
  **The second is the price**, on the same screen, and it behaves differently on purpose — a colour is replaced and a price is appended (K-14, above). Two settings on one school that do not behave alike is exactly the thing an operator will assume wrongly, so the screen says which is which rather than leaving it to be discovered.
  ...
  WHAT IS NOT GOING TO HAPPEN IS A PARAMETER REGISTRY. A table of names and values with a screen that edits any of them is the configuration surface K-13 exists to refuse: it grows to fill the space it is given, and every knob multiplies the state two people have to test. The two parameters that exist are the two that have no right answer — a school's colour and its price — and each got a control shaped like the thing it sets. What is still owed by this box is the LIST: somewhere that says these two are all there are, so that the next value somebody wants to make settable has to argue with a document rather than with nobody
