...
- [x] Staff roles — owner, operator, read-only, as a row on an account rather than a second account. *Invitations no longer wait on e-mail: the platform sends since v0.9.0, from a domain the provider has authenticated. What they wait on now is somebody writing them — a role is still granted from a terminal, by `cmd/staff`.*
...
- [x] A person can change the address on their account — **and it had to exist before anything is gated on confirmation, not after.** The suppression list could tell somebody their address had refused our mail; that was the whole of it, a true sentence about a problem with no remedy on any screen, shown to the one person who could do nothing about it.

  **The address moves when the new one is PROVED, not when it is typed.** A row is a change that has not happened, so a typo is a message nobody reads rather than an account nobody can reach — which is the failure this feature would otherwise be a new way to cause. It is a second table rather than a column on `account_email_confirmations`, because that table's redemption carries the condition `0035` calls the half of an address change that cannot be added afterwards: a token issued for one address must not confirm another. A change does the opposite on purpose, and merging them would mean loosening exactly that guard. A useful accident falls out: every confirmation link sitting in the OLD inbox dies the instant the address moves, because its own condition stops matching.

  The password is required, because a stolen cookie lets somebody read and moving where the recovery mail goes is what turns it into a stolen account. The other sessions are **not** ended — against first instinct: a password change does that because a credential changed, and the attacker who got this far already has the password anyway. The old address is told, **only on confirmation** and never on the request, which would otherwise be a way to post a message to any address by asking to move there and never clicking; a **suppressed** old address does not block the change and simply gets no notice, since that is the commonest reason to be here at all. And the notice carries no link and nothing to click, because that is the shape of every phishing mail, sent to somebody who may have just lost an account.

  **One abuse surface, named rather than discovered**: an authenticated session can now make this platform post to an address of the sender's choosing. Three an hour per account, with the threshold beside its reason
...
- [x] Section progress, resume pointer, notes — completion set-true and never toggled, and refused in a course the student cannot open. **A section that asks questions is finished by ANSWERING them**, and one that asks none by moving on: it was moving on either way, which put a certificate and the question "what did you think of this course" on top of a student having clicked `next` fourteen times. Answered and not answered correctly — see `A-16`
...
- [x] **Answering a lesson's own questions on a screen** — the route is `internal/lesson`, the third instrument and the smallest, because A-10 took the score away: it presents a lesson's questions with their keys removed and their order shuffled, marks one, gives the key back with the option's own `why`, and **stores nothing at all**. An exam question is refused by the `WHERE` clause and not only by a check afterwards. `TestThisPackageWritesNothing` scans the package's source for the four verbs, because a test that answered a question and looked for rows could only look in the tables somebody thought of

  **THIS BOX WAS NOT TICKED WHEN THE ROUTE LANDED, AND THAT TWO-HOUR GAP IS THE ITEM.** The route merged green. The screen merged green. Between them, the student could not answer a single question — and it took two more pull requests to find out why, both defects sitting in the space between two things that each worked and each had tests:

  **One.** `lessonSections` decided whether an assessment had questions by counting `window.SAMPLE_EXERCISES`, the predecessor's static sample data, which this interface never loads. Always zero, so every assessment in every lesson was `pending`, and the guard in `lesson.js` was false every time. `api.lessonExercises` was never called by anything. Fixed by the server counting — `LessonView.Questions`, which is the argument `Exam` already makes one type up: a screen has to be told rather than find out by being refused.

  **Two.** `buildAssessment` took `options.lesson` and built every question without it, so the marking fell through to `api.grade` with no attempt — which is `gradeLocally`, comparing an answer against a question the browser holds no key for. An option accepted at one end and dropped at the other.

  Neither was reachable by any test that existed. Seven store tests against a real database were green; a `fetch`-stubbed wiring harness was green. **Nothing was looking at the join**, and the screen that resulted looked exactly like a lesson whose material is not written yet — which is the honest state of 118 of the 122 courses, so it read as ordinary.

  **What earns the tick is the check that crosses the boundary**, in `a11y-test`: a browser signs up, opens the assessment, waits for the response from the route, fails if the list is empty or carries `"correct"` or `"why"`, answers a question and expects a verdict from the server. It asserts the REQUEST and not the pixels, because a screen rendering an empty assessment passes every contrast rule there is. It found the second defect while being written, and passed on its first real run

  **AND THE FIRST DEFECT HAD A SECOND HIDING PLACE, found months later.** `window.SAMPLE_EXERCISES` was also what "how you are doing" and "the ones you got wrong" joined a student's answers against — so both screens were permanently empty, on any account, however much had just been answered, and each said so in a polite sentence that reads exactly like a student who has not started. The two are repaired and neither is ticked here: they now join against the questions the sitting has actually been shown, and nothing automated holds them yet. The lesson of the global is not that it was missed twice; it is that an empty state and an empty screen are indistinguishable from outside, which is the same sentence this item already ends on
...
- [ ] Pix Automático, deliberately after the first two
  ...
  **THE SPLIT IS BY MECHANISM AND NOT BY COUNTRY, and getting that wrong was the first mistake made here.** "Annual and biennial, paid by Pix and instalments" was read as "Brazil has no recurrence", and it does not follow: a stored card that renews the subscription at the end of the term is recurrence, and a long term does not make the charge a one-off. Alura does exactly that. So:
  ...
  - **One-off** — Pix in a single payment, and the instalment authorisation. A term is bought; at its end, a new sale.

  **TWO OF THESE WERE WRITTEN FROM MEMORY AND THE SANDBOX SAID OTHERWISE.** Both were corrected here rather than discovered in code, which is the whole reason for opening an account before designing against one.

  *Debit is not a thing this gateway sells.* The methods offered on a charge are Pix, boleto and credit card — nothing else. The domestic list said "or debit" for a week; it does not any more. Debit is not lost so much as never available on this rail: whoever wants it is asking for a second provider, and that is a decision with a cost attached rather than a checkbox somebody forgot.

  *And there is no biennial cycle.* A recurring subscription's frequency stops at Anual — weekly, fortnightly, monthly, bimonthly, quarterly, half-yearly, yearly, and that is the list. So the two-year product cannot be a recurrence at all: it is ONE charge for a 24-month term, renewed as a new sale. This costs nothing to build, because it is exactly the model the instalment machine below already implements — but it means the annual and the biennial products are not the same shape with a different number, and any code that assumes they are will be wrong at the second one.
  ...
  *Instalments run to twelve, with no interest.* **This paragraph said six, and the estimate it said it on was wrong by more than six times.** It read "the published rate for twelve reaches upwards of twenty per cent of the sale"; the published table is **R$ 0,49 + 2,49% for two to six** and **R$ 0,49 + 2,99% for seven to twelve**, charged on the total of the sale. Half a point between the bands — on a term of R$ 690, **R$ 3,45**.

  It was written as a number to check and then treated as a number that was known, which is the failure this whole section warns about applied to itself. The rate is not in the charge screen; it is on the published fee page, where nobody looked until the cap had already been built.

  **AND WHAT SIX COST WAS NOT THE FEE.** Six instalments of a term cheaper than every rival still showed R$ 115 a month against Alura's 12× R$ 109 — a larger monthly figure on a product costing 47% less, and the monthly figure is what a buyer here compares. Twelve makes it R$ 57,50. That is what R$ 3,45 buys.

  Passing the interest on was considered properly rather than dismissed, and the real table is what settles it: recovering R$ 3,45 through a rate the buyer pays would add around **R$ 92** to a twelve-way split. That is a financing product, not a cost being covered. Three reasons stand behind that arithmetic: **selling with interest carries a disclosure obligation** under consumer law, which is real work and is not the work of getting somebody to pay; **it converts badly**, because the person who wants twelve is price-sensitive by definition and "12× R$ 65,21, total R$ 782" against "12× R$ 57,50, total R$ 690" is not a choice anybody makes; and **it divides two numbers this phase just joined**. With interest, the amount charged stops equalling `plan_prices`, and then RENEWAL MUST CHARGE THE PRICE AND NOT THE AMOUNT PAID — interest is a cost of that purchase, not part of the price, and a renewal that forgot it would bill R$ 782 to somebody who agreed to R$ 690.

  **AND THE CEILING WAS THE BROWSER'S ALONE UNTIL `MaxInstalments`,** which is a separate mistake from the rate and a worse one: the picker stopped at six and `Checkouts.Open` clamped only the bottom, so a request for five hundred instalments was accepted, written to `checkout_intents` and carried to the gateway to refuse. A limit only the form knows is a suggestion to whoever uses the form.

  If interest ever does arrive it brings one thing with it: **the rate table becomes effective-dated data, like the prices**, for K-14's reason exactly. "Why was I charged R$ 782" must stay answerable in November, and "the rate was different then, I think" is not an answer. The same applies to the fee table above the moment anything computes with it rather than quoting it.

  *Pix carries a five per cent discount, and it does not pay for any of itself.* **This paragraph claimed a third, and the arithmetic was done against the wrong price and the wrong lane.** On a term of R$ 690: the discount is **R$ 34,50**, and the fee it saves against a card at 1,99% + R$ 0,49 is **R$ 13,23**. It costs R$ 21,27 a sale, net. Put the three side by side and Pix is the option that delivers least:

  | | the buyer pays | we receive |
  |---|---:|---:|
  | Pix, less 5% | 655,50 | **654,51** |
  | card, à vista | 690,00 | 675,78 |
  | card, 12× | 690,00 | 668,88 |

  The discount is still worth it, and for one reason only, which is not the fee: **Pix settles in seconds and the card settles in 32 days**, per instalment. At twelve the last twelfth arrives about thirteen months after the sale. That is a cash-flow argument and it should be made as one — and if the cash is the problem, Asaas sells antecipação, which is the instrument for it and is chosen per need rather than imposed on every buyer.

  Treat every rate here as indicative and as **promotional**: the published page marks them "válido por 3 meses" and reverts to 2,99% à vista, 3,49% for two to six and 3,99% for seven to twelve. The half-point step between the bands is the same in both tables, so the decision above does not move — but the absolute numbers do, and this file has already been wrong once by quoting a rate it had not read. They are also the standard table rather than one anybody negotiated; the account's own contract is the authority.

  *That page also lists a debit card at R$ 0,35 + 1,89%, which this file declares does not exist. The declaration was made from the sandbox's charge screen, where the methods really are Pix, boleto and credit card — so this is most likely Asaas Tap rather than a `billingType` the API accepts. It is written down as unresolved rather than quietly corrected in either direction.*
...
*And the demonstration was itself unreliable for months, which nothing noticed because everything
that ran, ran on a seed that worked. Measured over forty seeds at the population the seeder
accepted, the planted key came back `inverted` NINE times: `enoughPeople` sized for the minimum
sample — where the index stops being dominated by which people sat the paper, not where it becomes
accurate — and the inversion was gentle enough to sit inside the error bars. Both are fixed and two
tests hold it across twenty-five seeds each. The lesson is narrower than "test the tests": a check
that runs once, on one input, is not a check on how often it is right.*

***Every box below is ticked and the phase is not finished***, which is the distinction phase 0
already draws. The second half of `Done when` — a drop at a step nobody suspected — cannot be
demonstrated by a population this repository invented, because a seeded funnel drops where the
...
- [x] Cohorts, by signup and by subscription start — **both, and they answer different questions.** By signup is *does the product hold the people it attracts*; by subscription start is *does it hold the people who paid*, which is a different population and the one whose retention costs money to get wrong. A school where signups stay and subscribers leave has a problem no signup cohort can show.
  ...
  **This box stayed open for a long time on the half that could not be built**, and the reason it could be closed had nothing to do with cohorts: nothing wrote a subscription into the stream, so grouping by one meant grouping by a moment nothing recorded. Billing emits `subscription.started` now, and three things that had each been described as their own gap turned out to be that one row — this, the funnel's last step, and the seeder's refund.

  **The second basis is still about ONE SCHOOL, which is the part that could have gone silently wrong.** A subscription covers every school (N-02), so it carries no tenant and the reader for it comes back with every subscriber on the platform. Membership stays school-scoped — somebody is in the table because they SIGNED UP here — and the subscription month only decides which row they are in. Folding the platform's rows in without that check would give every school the same intake, with every number plausible; the funnel's last step made exactly that mistake once, and a test now holds both.

  **And the first column follows the basis.** "The month they signed up" over a table of subscriptions is a heading that contradicts its own numbers, which is worse than a vague one because it is specific
...
- [x] The funnel, all eight steps from *arrived* to *subscribed* — **counted per PERSON, which is the only hard part.** The top is browsers and the bottom is accounts, so counting each step by whichever identity its event happened to carry would make somebody who arrived on Monday and subscribed on Friday two people, and the conversion rate a ratio between different populations. A person is an account where the identity is linked to one and the visitor otherwise, which is what `account_visitors` is for and why the visitor has an identity before the account exists (K-10). One person on two browsers is one person; forty lessons opened is one lesson step.
  ...
  **All eight are emitted now.** Two were not for a long time — verifying an address, then subscribing — and came back saying *no event exists yet* rather than zero, because a zero reads as everybody dropping out there, which is the same mistake as a discrimination index of zero that was never measured. `Measured` is still a field of its own: the distinction is what makes the screen safe to draw, not a note about the state of the product on a particular day.

  **The last step is the one that is not about a school**, and it says so on the row and under the chart. One subscription covers every school (N-02), so what it counts is the people who arrived HERE and went on to subscribe ANYWHERE — which is the question a funnel is opened to ask, and which means two schools can each count the same person. Their last steps do not add up to the platform's subscribers, and the screen carries a sentence saying so rather than leaving somebody to add them
...
- [x] What they are holding — phone, tablet or computer — **one of four words, never a width and never a pointer.** The server asks the browser first, through `Sec-CH-UA-Mobile` and `Sec-CH-UA-Platform`; those are sent by Chromium and by nothing else, so the interface answers as well, from the two things the layout itself responds to — a coarse pointer is a finger, and the width is which of the stylesheet's layouts is in use. Without that second source every iPhone on Safari and every Firefox lands as `unknown`, which is a hole big enough to make the whole number a report about Chromium wearing the word "students".

  **It is a dimension of the EVENT and not of the person**, which is why it rides on every request rather than being read once at the first touch: somebody who arrives on a phone and studies on a laptop is both, and a column on an account would have to pick one and be wrong about the other half of their evenings. The server bounds it to the same four words on the way in, because a header is the caller's.

  A window dragged across the breakpoint mid-evening is somebody resizing a window and not changing device, so the interface reads it once at load. The screen is `Measure → What they are on`, beside the countries for the reason its comment gives — one question about two dimensions of the same arrival. `internal/platform/device` is the one place a header is interpreted into a word, and the registry entry for `events` names the column: a dimension about the world rather than about a person, **deliberately too coarse to recognise anybody by**, which is what keeps a new column in an orphan-surviving table from becoming the thing that un-orphans it
...
- [x] Email deliverability — bounces and complaints — **the mechanism, and not the screen; the box is ticked on what it says and `docs/CONSOLE.md` still lists the screen among what is not built.** The platform started sending in v0.9.0 and was deaf until v0.10.0: an address that refused us permanently went on being written to, and every attempt after the first is a mark against this domain with the providers who decide whether ANYBODY else's mail arrives. One unreachable address costs one person a link; a hundred attempts at it costs every future student their inbox.

  **A person in this state is now told, and can act.** The banner says the address refused our mail and offers no button, because a control that cannot work invites somebody to keep pressing and the send is refused anyway — and the account screen can move the account to an address that works. That pair took two changes after this one and belongs to the same sentence: knowing, saying, and being able to fix it.

  **The list holds a hash and never an address**, which is the whole design rather than a detail of it. A suppression list is the one record that has to survive an erasure — somebody asks to be forgotten, signs up a month later with the same mailbox, and we write to one that already refused us — and keeping the address would be keeping a person's data after they asked us not to. The two obligations look irreconcilable and are not: what the table has to answer is *may I write to THIS address*, and a SHA-256 answers it exactly while holding nothing that was erased. It is `pseudonymous` in the registry, `EraseKeep`, and both privacy policies say so in the words a person reads.

  **Only what is permanent**, checked at the hook and again at the list. A soft bounce is not a refusal, and treating one as permanent would have suppressed every address at a whole provider during the Proton outage of 27 August 2026 — which is not a hypothetical, it is the day before this shipped. Unsubscribing is absent for its own reason: refusing somebody the link that proves they own their own address, because they once opted out of a newsletter that does not exist, would be obeying an instruction nobody gave.

  NOTHING SIGNS A DELIVERY EVENT, so a shared secret is all there is, and **it travels in a header — after a first version that put it in the path.** That version was built on the belief that the provider offered nothing better, written into four files in capital letters before anybody opened the provider's own form, which offers HTTP Basic. What the move bought is three places the secret no longer appears: the request log (`web.Logger` writes `r.URL.Path` on every request), the address bar of whoever configures it, and a screenshot of that screen — which is how sixteen characters of the first one reached a chat transcript. Basic and not the provider's own "Token", because Basic means the same everywhere and the other is a header whose shape is the provider's to change, which is the mistake being corrected rather than repeated.

  **The redaction under `/hooks/` stayed anyway**, though nothing secret is in a path now: the payment gateway arrives under the same prefix from a provider nobody has read yet, and the change that removed it would be the one somebody later puts a secret back into a path under.

  **And a fourth reason, `invalid`** — the provider refusing to try at all. It is unlike the other three, which is why it was missed rather than excluded: nothing left, so nothing was held against this domain. What it buys is the rest of the value — attempts not spent, and a support answer that says "that address does not exist" instead of "I cannot tell you why nothing arrived". `0038` widened a CHECK that lists its values by name, which is the property that made a fourth one a decision rather than an accident
...
- [x] What the student thinks of a course, a track and the platform — **five stars, and a tap on one is a complete answer.** It is the sibling of the queue above and every decision in it comes from being the opposite: a report is a `<details>` closed until somebody has found a defect and knows it, this is open, visible and asked of everybody, so it has to survive being ignored and be finishable in one gesture by somebody who does not want to be asked anything. The stars are five radios sharing a legend, not five spans with click handlers, which is what makes them arrow-key operable and announced (`X-05`, `X-06`) — "★★★" read aloud is not a rating.

  **The four pairs are asked AFTER, and sometimes not at all.** Somebody who taps five has told us what they think; somebody who taps two has the information, and four short questions is a fair thing to ask of them and not of everybody. The threshold arrives with the lists from the server rather than being copied into the interface, so moving it moves what is asked (`K-16`).

  **Nothing here has a text box**, and that is the boundary between this feature and the queue above: a comment box turns counting opinions into reading them one at a time. Somebody with something to SAY has the control on every section for exactly that.

  It is asked once a LESSON of the course is finished, and once a COURSE of the track is — not at the end of either. `web-fundamentals` is ninety-four sections, and a question asked there arrives months after the lesson it is about, from somebody who has stopped being the beginner who could have said the third lesson moved too fast. The screen is `Measure → What they think of it`, the rows are `ratings` in the privacy registry and in `cmd/reset`'s list of what a reset empties, and `a11y-test` measures both ends — the widget opened to its second half on the account screen, and the console screen with a row in it
- [x] The closed list of system parameters, each change audited with actor, old and new value — **and the list is in Go with a test on it, because the obvious build was the wrong one.** A school's accent is set from `Operate → Schools`, recorded with the actor, the colour that was there and the colour that replaced it, and refused for a role below operator. The audit seam grew to make that possible: it carried one value and a bare account id, so every entry the console wrote said "account" and put whatever it had in `before`; it now names the SUBJECT'S KIND and both sides of the change.
  ...
  **The second is the price**, on the same screen, and it behaves differently on purpose — a colour is replaced and a price is appended (K-14, above). Two settings on one school that do not behave alike is exactly the thing an operator will assume wrongly, so the screen says which is which rather than leaving it to be discovered.
  ...
  WHAT DID NOT HAPPEN IS A PARAMETER REGISTRY — **until `0046`, which built it; the paragraph below is what was true when this item shipped and the amendment follows it.** A `system_parameters` table — a name, a value, a screen that edits any row — is the shape that suggests itself and precisely the configuration surface K-13 exists to refuse: it grows to fill the space it is given, and the next value somebody wants to make settable costs one INSERT and no argument.
  ...
  **AND THAT LAST SENTENCE IS WHERE THE REFUSAL STOPPED WORKING.** The mechanism above makes a knob cost a migration, and it was right to at three. It is wrong at fifteen: a platform asked to make eleven more numbers changeable without a deployment would pay eleven migrations, and the eleventh sentence is written by somebody who has stopped meaning it. The cost was in the wrong place, not the wrong size. `0046` moves it onto the DECLARATION — a `setting.Declared` in the module that owns the decision, with its unit, the bounds it may move inside, the value the code ships with and the argument for it existing at all — and `cmd/api`'s `TestEveryParameterCarriesItsArgument` fails one without an argument, without room to move, or with a fallback its own fence would refuse. A name declared twice panics at start-up; a name declared nowhere is a 404 on the way in and decides nothing on the way out. The console gets ONE route and one screen for all of them, and it can only draw what something declared: there is no way to add a parameter from a screen, and there will not be.

  What did not move: nothing about ACCESS may be declared (reads come from a snapshot up to fifteen seconds old, and K-15 keeps the paywall out of every parameter surface anyway), and neither may anything whose wrong value is a WEAKENING rather than a preference — a minimum password length, a recovery-code count, the cost of the hash. Those have right answers, and a settable minimum is a weakening with an interface on it.
...
- [x] Prices effective-dated — a subscriber keeps the price they bought at **for the term they bought**. *Those last five words were once implicit, were read as "for ever", and reached a column comment in production that contradicted the terms of use — `0040` corrects it, and K-14 now carries the qualifier.* **Both halves now: the offer is a series, and the subscription points at a row of it.** `tenants.plan_price_cents` was a column that could be overwritten, which reads as harmless and is the one shape a money parameter may not have: the moment it changed, "why was this person charged 490 when the site says 590" stopped being answerable. `ledger_entries` has been append-only since it existed, so what somebody PAID was already unforgeable; this was the other half of that sentence and it was the editable half.
  ...
  `subscriptions.price_id` is the half that closes it, and it is NOT NULL — which was only possible on the day it was added. **On that day** nothing created a subscription: there was no gateway, the seeder wrote none, and `billing.Begin` had no caller outside its own tests. A year from now the choice would have been a nullable column half the code checks, or a backfilled guess about what somebody was charged — the one guess this schema exists to prevent. So the migration REFUSES rather than backfills: a `DO` block raises if the table has rows, and says what to decide.

  **Those two words are a correction and not a flourish.** The sentence was written in the present tense about a day that has passed, and the gateway arriving made it false while it went on reading as a description of now — `funnel.go` says in its own comment that billing "has created subscriptions for weeks". The window the paragraph describes was real and is the whole argument for NOT NULL; what it may not do is claim to still be open.

  WHICH ROW: **the one in force for the term being bought**, and that sentence took two migrations. `0036` had to say "the one in force at the school where the purchase happened", because a subscription was platform-wide (N-02) while a price was a school's — honest bookkeeping, and no defence at all: with `code.` at 590 and `math.` at 390 the rational purchase is through `math.`, and it opens both. `0041` raised the price to what it prices. `school_prices.tenant_id` became `plan_prices.scope`, spelled exactly as `subscriptions.scope` is, so the two are compared rather than reconciled by a rule somebody remembers — and when scope narrows (N-03) both halves already say the same word.
  ...
  AND A PRICE SAYS WHAT IT BUYS. `term_months` arrived with the scope, because the plan is annual, biennial and — abroad — monthly, and one number per scope cannot say which of those it is. It is a NUMBER and not a name: `paid_through` is that many months on, and a name would only have to be turned back into the number the date is computed from. The rows that existed became 12 without a guess, because the interface has quoted this price as "per year" since before it was a table.

  The sentence in the title is closed. What it waited for was the gateway, and the two blockers named here are both gone: the scope question is `0041`'s, and the link a renewal reads is what `0036` and `0040` settled between them
...
- [x] A seeder that generates **history** — months of backdated events, with abandonment, returns, duplicate signups and refunds. **All four.** `cmd/seed` writes a past into the event stream, which is the only thing it backdates, because statistics come from the stream and never from current state. The refund was the one that could not be represented until a subscription reached the stream; it is `subscription.started` followed by `subscription.ended` carrying `refunded`, and the person's later events go back to saying `none`.

  **Adding it broke the exam fixture, which is the part worth remembering.** The refund was drawn from the same generator as everything else, so every draw after it shifted: the same seed produced a different population and a test failed on a question nothing had touched. A fixture other tests assert against cannot reshuffle whenever an unrelated behaviour is added, so the subscription draws have their own sequence from the same `--rand`, and the exam fixture is byte-identical to what it was.

  **And chasing that turned up a defect underneath it.** The command plants an inverted answer key and errors out when the analysis fails to find it — that is this phase's `Done when` asked as a question every run answers. Measured over forty seeds at the population the command accepted, it failed THIRTY-ONE TIMES. `enoughPeople` sized for the minimum sample, which is where the index stops being dominated by which people sat the paper and not where it becomes accurate; and the planted inversion was gentle enough to sit inside the error bars. Both are fixed and two tests hold it over twenty-five seeds each, in a fifth of a second, because the model is a pure function of a seed. Nothing had caught it because everything that ever ran, ran on a seed that worked
