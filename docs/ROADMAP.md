...
- [ ] Every threshold displayed beside the number it produced — **the item analysis does this and the exam does not.** All six of that screen's bars come from `analysis`'s own constants, on the answer, so the screen cannot hold a stale copy of a decision. `item_statistics` stores `minimum_sample` on the row for the same reason one level down: a verdict computed under a minimum of thirty and displayed beside a constant that now says fifty is a row explaining itself with the wrong number.

  What is left is the pass mark. The server applies `exam.PassMark`, stores it on the attempt and sends it as `pass_mark` — and `ui/app/exams.js` declares `PASS_MARK = 70` of its own and shows THAT, twice: on the page before the exam and beside the result. It is the defect this box exists to prevent, sitting in the interface, and it is invisible until somebody changes the constant on one side
