...
| C-22 | The release's level is a computed floor, raisable with a reason and never lowered | major is the structure, minor an exercise or a video, fix the prose — the three classes of artefact by what records them, which is derivable where "how big was the change" is not. But a corrected accent and a rewritten section raise the prose version identically, because the difference is intent and no diff carries intent. So the tool guarantees the floor and a person may declare higher, which leaves the number incapable of understating — the dangerous direction — and puts judgement only where the machine is blind, including content redistributed among the same sections. `A-04` already derives rather than asks. |
