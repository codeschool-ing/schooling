...
| An `operator` may approve and publish a release | Three roles and no more (`K-01`): `owner` does everything including granting roles, `operator` changes a plan and quarantines a question, `read-only` writes nothing. Publishing is the same shape as quarantining — an audited write that changes what students see — so it needs no fourth role, and owner-only would make the one person who grants roles the only one who can ship. | Proposed |
