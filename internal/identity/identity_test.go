...
func TestASessionIdentifiesItsAccountUntilItIsRevoked(t *testing.T) {
	...
	got, _, err := store.Verify(ctx, token, nil)
	if err != nil {
	...
	if _, _, err := store.Verify(ctx, "not-a-token", nil); !errors.Is(err, identity.ErrNoSession) {
...
func TestSigningOutEndsOneSessionAndNotTheOthers(t *testing.T) {
	...
	if _, _, err := store.Verify(ctx, phone, nil); err != nil {
...
func TestChangingThePasswordEndsEveryOtherSession(t *testing.T) {
	...
	if _, _, err := store.Verify(ctx, theirs, nil); !errors.Is(err, identity.ErrNoSession) {
	...
	if _, _, err := store.Verify(ctx, mine, nil); err != nil {
